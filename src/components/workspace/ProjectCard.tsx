...
export function ProjectCard({ project, onEdit, onRefresh }: ProjectCardProps) {
  ...
  const handleDelete = async (e: React.MouseEvent) => {
    ...
    if (
      !window.confirm(
        `Are you sure you want to delete project "${project.name}"? This will delete all linked configuration.`,
      )
    ) {
      return;
    }
        ...
        <div className="space-y-1 min-w-0 flex-1">
          <div className="flex items-center gap-2">
            ...
            <Badge
              variant="outline"
              className={cn(
                'px-1.5 py-0 text-[9px] uppercase font-extrabold tracking-wider shrink-0',
                getEnvBadgeColor(project.environment),
              )}
            >
              {project.environment}
            </Badge>
        ...
        {isAdmin && (
          <div onClick={(e) => e.stopPropagation()} className="ml-2 shrink-0">
            <DropdownMenu>
              ...
              <DropdownMenuContent
                align="end"
                className="bg-card border border-border text-foreground rounded-xl shadow-md p-1 min-w-[130px]"
              >
                <DropdownMenuItem
                  onClick={onEdit}
                  className="hover:bg-neutral-100 dark:hover:bg-neutral-900 dark:hover:text-white text-xs gap-2 py-1.5 rounded-lg cursor-pointer"
                >
                ...
                <DropdownMenuItem
                  ...
                  className="hover:bg-neutral-100 dark:hover:bg-neutral-900 text-xs text-red-500 hover:text-red-550 dark:hover:text-red-400 gap-2 py-1.5 rounded-lg cursor-pointer"
                >
