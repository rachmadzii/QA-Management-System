...
const getUserRole = (userData: any) => {
  ...
  return (
    userData.role || userData.userRole || userData.profile?.role || roles[0]
  );
};
...
export function BugDialog({
  ...
  const selectedEndpointId = useWatch({
    control,
    name: 'endpointId',
  });
  ...
  const onSubmit = async (values: BugFormValues) => {
    ...
    if (profile.role !== 'qa' && profile.role !== 'admin') {
      toast.error(
        'Unauthorized: Only users with the QA role can report or edit bugs.',
      );
      return;
    }
    ...
    try {
      ...
      const bugData = {
        ...
        stepsToReproduce: config.stepsToReproduce.enabled
          ? values.stepsToReproduce || null
          : null,
        expectedResult: config.expectedResult.enabled
          ? values.expectedResult || null
          : null,
        actualResult: config.actualResult.enabled
          ? values.actualResult || null
          : null,
        severity: config.severity.enabled ? values.severity || null : null,
        priority: config.priority.enabled ? values.priority || null : null,
        httpStatus:
          config.httpStatus.enabled && values.httpStatus
            ? parseInt(values.httpStatus)
            : null,
        endpointId: config.endpointId.enabled
          ? values.endpointId || null
          : null,
        endpointPath:
          config.endpointId.enabled && selectedEp
            ? selectedEp.path || null
            : null,
        endpointMethod:
          config.endpointId.enabled && selectedEp
            ? selectedEp.method || null
            : null,
        swaggerLink:
          config.endpointId.enabled && selectedEp
            ? selectedEp.swaggerLink || null
            : null,
        assignedTo: config.assignedTo.enabled
  ...
  const selectedEndpointObj = endpoints.find(
      ...
      <DialogContent className="sm:max-w-[720px] max-h-[90vh] flex flex-col bg-card border border-border text-foreground rounded-2xl shadow-xl p-0 overflow-hidden">
        ...
        <DialogHeader className="p-6 pb-4 border-b border-border/80 bg-neutral-50/20 dark:bg-neutral-900/10 shrink-0">
          ...
          <DialogDescription className="text-muted-foreground text-xs leading-relaxed mt-1">
            Provide details of the API failure, associate it with a specific
            endpoint, and attach diagnostic evidence.
          </DialogDescription>
        </DialogHeader>
            ...
            <div className="bg-red-500/10 border border-red-500/25 p-4 rounded-xl flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1 leading-relaxed font-semibold">
                  Your role is set to <strong>{profile?.role}</strong>. Only QA
                  engineers are allowed to log or modify bugs.
                </p>
              </div>
            </div>
          </div>
                        ...
                        <div className="grid grid-cols-4 gap-1.5">
                          {[
                            ...
                            },
                            {
                              ...
                              dot: 'bg-orange-500',
                              active:
                                'bg-orange-500/10 text-orange-400 border-orange-500/35 shadow-[0_0_8px_rgba(245,158,11,0.15)]',
                            },
                            {
                              key: 'minor',
                              label: 'Minor',
                              dot: 'bg-yellow-500',
                              active:
                                'bg-yellow-500/10 text-yellow-400 border-yellow-500/35 shadow-[0_0_8px_rgba(234,179,8,0.15)]',
                            },
                            {
                              key: 'low',
                              label: 'Low',
                              dot: 'bg-neutral-400',
                              active:
                                'bg-neutral-100 dark:bg-neutral-900 border-neutral-300 dark:border-white/10 text-neutral-600 dark:text-neutral-300',
                            },
                          ].map((item) => {
                            ...
                            return (
                              <button
                                ...
                                type="button"
                                onClick={() =>
                                  setValue('severity', item.key as any)
                                }
                                className={cn(
                        ...
                        <div className="grid grid-cols-3 gap-1.5">
                          {[
                            ...
                            },
                            {
                              ...
                              label: 'Low',
                              active:
                                'bg-neutral-100 dark:bg-neutral-900 border-neutral-355 dark:border-white/10 text-neutral-600 dark:text-neutral-300',
                            },
                          ].map((item) => {
                            ...
                            return (
                              <button
                                ...
                                type="button"
                                onClick={() =>
                                  setValue('priority', item.key as any)
                                }
                                className={cn(
              ...
              <div className="space-y-4">
                ...
                {config.stepsToReproduce?.enabled && (
                  <div className="space-y-1.5">
                    ...
                    >
                      Steps to Reproduce{' '}
                      {config.stepsToReproduce?.required && (
                        <span className="text-red-500">*</span>
                      )}
                    </Label>
                    <Textarea
                  ...
                  <div className="grid grid-cols-1 gap-4">
                    {config.expectedResult?.enabled && (
                      <div className="space-y-1.5">
                        ...
                        >
                          Expected Result{' '}
                          {config.expectedResult?.required && (
                            <span className="text-red-500">*</span>
                          )}
                        </Label>
                        <Textarea
                      ...
                      <div className="space-y-1.5" ref={dropdownRef}>
                        ...
                        <div className="relative">
                          <button
                            type="button"
                            onClick={() =>
                              setEndpointSearchOpen(!endpointSearchOpen)
                            }
                          ...
                          >
                            {selectedEndpointObj ? (
                              <div className="flex items-center gap-2 truncate">
                                <span
                                  className={cn(
                                    'px-1.5 py-0.2 rounded font-mono text-[9px] font-extrabold tracking-wide',
                                    getMethodColor(selectedEndpointObj.method),
                                  )}
                                >
                                  {selectedEndpointObj.method}
                                </span>
                                <span className="font-mono text-[11px] text-foreground truncate">
                                  {selectedEndpointObj.path}
                                </span>
                              </div>
                            ) : (
                                        ...
                                        >
                                          <div className="flex items-center justify-between w-full">
                                            <div className="flex items-center gap-2 truncate">
                                              <span
                                                className={cn(
                                                  'px-1 py-0.2 rounded font-mono text-[8px] font-extrabold',
                                                  getMethodColor(ep.method),
                                                )}
                                              >
                                                {ep.method}
                                              </span>
                                              <span className="font-mono text-[11px] truncate text-foreground group-hover:text-sky-500 transition-colors">
                                                {ep.path}
                                              </span>
                      ...
                      <div className="space-y-1.5" ref={assigneeDropdownRef}>
                        ...
                        <div className="relative">
                          <button
                            type="button"
                            onClick={() =>
                              setAssigneeSearchOpen(!assigneeSearchOpen)
                            }
                            className="flex h-9 w-full items-center justify-between rounded-xl border border-neutral-250/20 dark:border-white/5 bg-neutral-100/50 dark:bg-neutral-900/60 px-3 py-2 text-xs text-foreground focus:outline-none focus:ring-2 focus:ring-sky-500/10 cursor-pointer"
                          >
                            {selectedAssigneeObj ? (
                    ...
                    {screenshotUrls.length > 0 && (
                      <div className="grid grid-cols-3 sm:grid-cols-4 gap-3 mt-3">
                        {screenshotUrls.map((url, idx) => (
                          <div
                            key={idx}
                            className="relative group rounded-2xl overflow-hidden aspect-video border border-neutral-250/20 dark:border-white/5 bg-card flex items-center justify-center"
                          >
                            <Image
                              src={url}
                              alt="screenshot"
