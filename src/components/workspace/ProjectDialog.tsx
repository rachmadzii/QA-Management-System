...
const projectSchema = z.object({
  ...
  baseUrl: z.string().url("Must be a valid URL starting with http/https").or(z.literal("")),
  swaggerUrl: z.string().url("Must be a valid URL starting with http/https").or(z.literal("")),
...
export function ProjectDialog({ open, onOpenChange, projectToEdit, onSuccess }: ProjectDialogProps) {
  const { user } = useAuth();
      ...
      <DialogContent className="sm:max-w-[480px] bg-card border-border text-foreground rounded-2xl shadow-lg p-6">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 py-2 text-xs">
          <div className="space-y-1.5">
            <Label htmlFor="name" className="text-muted-foreground font-semibold">Project Name</Label>
            <Input
              id="name"
          ...
          <div className="space-y-1.5">
            <Label htmlFor="swaggerUrl" className="text-muted-foreground font-semibold">Swagger / OpenAPI URL</Label>
            <Input
              id="swaggerUrl"
            ...
            />
            {errors.swaggerUrl && (
              <p className="text-xs text-red-500 font-semibold mt-1">{errors.swaggerUrl.message}</p>
            )}
          </div>
